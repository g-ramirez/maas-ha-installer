...
def get_container_ip(container_name):
    ...
    cmd = '''lxc list %s -c 4 --format csv |sed \'s/\"//\'|grep eth0|awk '{print $1}'|head -1''' % container_name
...
'''
    ...
    for i in range(1,4):
        ...
        # configure pg stat
        run('''lxc exec maas-snap-%s -- sh -c \'echo "d /var/run/postgresql/12-main.pg_stat_tmp 2750 postgres postgres" > /etc/tmpfiles.d/1-main.pg_stat_tmp.conf\'''' % i)
...
def sync_slaves_if_not():
    ...
    cmd1 = "lxc exec maas-snap-1 -- sh -c 'sudo crm status|grep Stopped|grep maas-snap-2'"
    output1 = run(cmd1) 
    if output1:
        ...
        sleep(10)
        cmd_sync2 = "lxc exec maas-snap-3 -- sh -c 'sudo rm -rf /var/lib/postgresql/12/main/* && sudo -u postgres pg_basebackup -h %s -D /var/lib/postgresql/12/main -v --wal-method=stream'" % vip_addr
        run(cmd_sync2)
        run('lxc exec maas-snap-3 sudo crm resource cleanup pgsql')
...
crm configure primitive res_maas_vip ocf:heartbeat:IPaddr2 \
    ...
    migration-threshold=0 
crm configure primitive haproxy lsb:haproxy op monitor interval=15s
crm configure clone haproxy-clone haproxy''' % (vip_addr, cidr)
    with tempfile.NamedTemporaryFile(mode="w", delete=False) as tmp_file:
        tmp_file.write(template)    
