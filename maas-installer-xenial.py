...
def create_containers():
    ...
    cmd = "lxc launch ubuntu:xenial maas-snap-1 -p maas && lxc launch ubuntu:xenial maas-snap-2 -p maas  && lxc launch ubuntu:xenial maas-snap-3 -p maas"
    try:
        run(cleanup)
    except:
        pass
...
'''
    ...
    for i in range(1,4):
        ...
        lxd_cp_command = 'lxc exec maas-snap-%s sudo cp /home/ubuntu/postgresql.conf /etc/postgresql/9.5/main/postgresql.conf' % (i)
        run(lxd_push_command)
        run(lxd_cp_command)
        # configure pg_hba.conf
        replication_cmd="lxc exec maas-snap-%s -- sh -c 'echo host    replication    postgres     %s    trust |sudo tee -a /etc/postgresql/9.5/main/pg_hba.conf'"  \
        % (i, subnet_addr)
        access_cmd="lxc exec maas-snap-%s -- sh -c 'echo host    maasdb    maas     %s   md5 |sudo tee -a /etc/postgresql/9.5/main/pg_hba.conf'"  \
        % (i, subnet_addr)  
        run(replication_cmd)
        run(access_cmd)
        run("""lxc exec maas-snap-%s -- sh -c 'sed -i "s/\\^node/node/g" /usr/lib/ocf/resource.d/heartbeat/pgsql'""" % i)
        # configure pg stat
        run('''lxc exec maas-snap-%s -- sh -c \'echo "d /var/run/postgresql/9.5-main.pg_stat_tmp 2750 postgres postgres" > /etc/tmpfiles.d/1-main.pg_stat_tmp.conf\'''' % i)
        run("lxc exec maas-snap-%s systemd-tmpfiles --create" % i)

        if i == 1:
            copy_cmd = "lxc exec maas-snap-1 -- sh -c 'sudo cp /etc/postgresql/9.5/main/pg_hba.conf /var/lib/postgresql/9.5/main/.'"
            copy_cmd2 = "lxc exec maas-snap-1 -- sh -c 'sudo cp /etc/postgresql/9.5/main/pg_ident.conf /var/lib/postgresql/9.5/main/.'"
            chown_cmd = "lxc exec maas-snap-1 -- sh -c 'sudo chown postgres:postgres /var/lib/postgresql/9.5/main/pg_hba.conf'"
            chown_cmd2 = "lxc exec maas-snap-1 -- sh -c 'sudo chown postgres:postgres /var/lib/postgresql/9.5/main/pg_ident.conf'"
            install_cmd = "lxc exec maas-snap-1 -- sh -c 'install -o postgres -g postgres -m 0700 -d /var/lib/postgresql/9.5/main/tmp'"
            run(copy_cmd)
            run(chown_cmd)
            run(copy_cmd2)
            run(chown_cmd2)
        ...
        manual_cmd="lxc exec maas-snap-%s -- sh -c 'echo manual |sudo tee /etc/postgresql/9.5/main/start.conf'"  % (i)
        run(manual_cmd)
...
def sync_postgres_slaves(vip=None):
    ...
    delete_cmd1 = "lxc exec maas-snap-2 -- sh -c 'sudo rm -rf /var/lib/postgresql/9.5/main/*'"
    delete_cmd2 = "lxc exec maas-snap-3 -- sh -c 'sudo rm -rf /var/lib/postgresql/9.5/main/*'"
    sync_cmd1 = "lxc exec maas-snap-2 -- sh -c 'sudo -u postgres pg_basebackup -h %s -D /var/lib/postgresql/9.5/main -v --xlog-method=stream'" % (master_ip)
    sync_cmd2 = "lxc exec maas-snap-3 -- sh -c 'sudo -u postgres pg_basebackup -h %s -D /var/lib/postgresql/9.5/main -v --xlog-method=stream'" % (master_ip)
    run(stop_cmd1)
    run(stop_cmd2)
    run(delete_cmd1)
    run(delete_cmd2)
...
crm configure primitive pgsql ocf:heartbeat:pgsql \
  params rep_mode="sync" \
    ...
    config="/etc/postgresql/9.5/main/postgresql.conf" logfile="/var/log/postgresql/postgresql-10-ha.log" \
    master_ip="%s" \
...
def sync_slaves_if_not():
    ...
    output1 = run(cmd1) 
    if output1:
        cmd_sync = "lxc exec maas-snap-2 -- sh -c 'sudo rm -rf /var/lib/postgresql/9.5/main/* && sudo -u postgres pg_basebackup -h %s -D /var/lib/postgresql/9.5/main -v --xlog-method=stream'" % vip_addr
        run(cmd_sync)
        run('lxc exec maas-snap-2 sudo crm resource cleanup pgsql')
        sleep(10)
    output2 = run(cmd2) 
    if output2:
        cmd_sync = "lxc exec maas-snap-3 -- sh -c 'sudo rm -rf /var/lib/postgresql/9.5/main/* && sudo -u postgres pg_basebackup -h %s -D /var/lib/postgresql/9.5/main -v --xlog-method=stream'" % vip_addr
        run(cmd_sync)
        run('lxc exec maas-snap-3 sudo crm resource cleanup pgsql')
